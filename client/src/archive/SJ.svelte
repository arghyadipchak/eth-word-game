<script lang="ts">
  ...
  async function createGame() {
    createButton = true
    const singerinstance = factoryInst.connect(provider.getSigner())
    const temp = await singerinstance.newGame($deployerAddress)

    factoryInst.on('NewGame', (game, event) => {
  ...
  async function startGame() {
    ...
    await singerGameInstance.startGame()
  }

  async function joinGame() {
    const singerinstance = factoryInst.connect(signer)
