<script lang="ts">
  ...
  try {
    metaMask = ethereum.isMetaMask === true
    ethereum
      .request({ method: 'eth_requestAccounts' })
      .then(accs => (currentAddress = accs[0]))
      .catch(() => {})

    ethereum.on('accountsChanged', accs => (currentAddress = accs[0] || ''))
  } catch (_) {
    ...
    <div class="dropdown dropdown-end">
      ...
      >
        <li>
          <!-- svelte-ignore a11y-missing-attribute -->
          <a
            >Account: {currentAddress.slice(0, 5)}...{currentAddress.slice(-4)}
          </a>
        </li>
      </ul>
    </div>
