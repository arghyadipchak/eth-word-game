...
contract('WordGame: let us play', acc => {
  it('game check', async () => {
    ...
    assert.equal(
      await game.methods.isLastFirstSame('air', 'rice').call(),
      true,
      'isLastFirstSame failed'
    )
    assert.equal(
      await game.methods.isLastFirstSame('air', 'like').call(),
      false,
      'isLastFirstSame failed'
    )
    ...
    assert.equal(
      await game.methods.checkIfPlayer(acc[0]).call(),
      true,
      'acc[0] is NOT a player'
    )

    await game.methods.joinGame().send({ from: acc[1] })
    assert.equal(
      await game.methods.checkIfPlayer(acc[1]).call(),
      true,
      'acc[1] is NOT a player'
    )

    await game.methods.joinGame().send({ from: acc[2] })
    assert.equal(
      await game.methods.checkIfPlayer(acc[2]).call(),
      true,
      'acc[2] is NOT a player'
    )
    ...
    assert.equal(
      await game.methods.gameStarted().call(),
      false,
      'other guy was able to start game'
    )
    ...
    assert.equal(
      await game.methods.gameStarted().call(),
      true,
      'acc[0] NOT was able to start game'
    )
    ...
    assert.equal(
      await game.methods.getWord().call(),
      'abdakdabra',
      'acc[1] was able to send word when not his turn'
    )
    ...
    assert.equal(
      await game.methods.getWord().call(),
      'abdakdabra',
      'acc[0] was able to send wrong word'
    )
    ...
    assert.equal(
      await game.methods.getWord().call(),
      'abs',
      'acc[0] was NOT able to send correct word'
    )
    //acc[1]'s turn should come
    assert.equal(
      await game.methods.getTurn().call(),
      1,
      'turn of acc[1] did NOT come'
    )
    ...
    assert.equal(
      await game.methods.hasGameEnded().call(),
      true,
  ...
  it('leave game tests: some people leave', async () => {
    ...
    assert.equal(
      await game.methods.getWord().call(),
      'story',
      'acc[2] was NOT able to send word'
    )
    ...
    assert.equal(
      await game.methods.getWord().call(),
      'yak',
      'acc[0] was NOT able to send word'
    )
  ...
  it('leave game tests: some people leave 2', async () => {
    ...
    assert.equal(
      await game.methods.getWord().call(),
      'story',
      'acc[1] was NOT able to send word'
    )
  ...
  it('repeat word tests: 1 guy repeats words', async () => {
    ...
    assert.equal(
      await game.methods.getTurn().call(),
      1,
      'acc[1] was able to send REPEAT word'
    )
